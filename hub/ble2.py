...
class PnPIdCharacteristic(Characteristic):
    ...
    def __init__(self, bus, index, service):
        ...
        # Vendor ID (0x0457 = Mouse-specific vendor ID)
        # Product ID (0x0001 = Mouse product ID)
        # Product Version (0x0100 = 1.0)
        self._value = [
            dbus.Byte(0x02), # Vendor ID Source: USB
            dbus.Byte(0x57), dbus.Byte(0x04), # Vendor ID: 0x0457 (Mouse-specific)
            dbus.Byte(0x01), dbus.Byte(0x00), # Product ID: 0x0001 (Mouse)
...
class TestAdvertisement(Advertisement):
    ...
    def __init__(self, bus, index):
        ...
        # 장치 이름 "BLE_Mouse"로 설정
        self.local_name = "BLE_Mouse"
        # 장치 외형을 마우스로 설정
