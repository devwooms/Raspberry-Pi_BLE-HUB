...
class HIDService(Service):
    def __init__(self, bus, index):
        ...
        self.add_characteristic(HIDReportMapCharacteristic(bus, 1, self))
        self.add_characteristic(HIDReportCharacteristic(bus, 2, self))
        self.add_characteristic(HIDProtocolModeCharacteristic(bus, 3, self))
...
class HIDInformationCharacteristic(Characteristic):
    def __init__(self, bus, index, service):
        Characteristic.__init__(
            ...
            ['read'],
            service)
        self.value = [0x11, 0x01, 0x00, 0x00]  # bcdHID, bCountryCode, Flags
...
class HIDReportCharacteristic(Characteristic):
    def __init__(self, bus, index, service):
        Characteristic.__init__(
            ...
            ['read', 'write', 'notify'],
            service)
        self.value = [0] * 8  # 키보드 입력 버퍼
        self.mouse_value = [0] * 4  # 마우스 입력 버퍼
    ...
    def send_mouse_report(self, buttons, x, y, wheel):
        self.mouse_value = [buttons, x, y, wheel]
        self.PropertiesChanged(GATT_CHRC_IFACE, {'Value': self.mouse_value}, [])
